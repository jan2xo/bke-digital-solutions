...
export default async function AccountPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section className="shell py-14"><p className="font-bold text-[#0b7197]">{account.type} · {account.effectiveRole}</p><h1 className="mt-2 text-4xl font-black">{account.displayName}</h1>{account.lifecycleState !== "ACTIVE" && <p className="mt-3 rounded border border-amber-400 bg-amber-50 p-3 font-bold text-amber-900">This account is {account.lifecycleState.toLowerCase()}. New purchases, renewals, downloads, key reveal, trials, and activations are disabled.</p>}<div className="mt-10 grid gap-8">
    ...
    <section className="grid gap-6 lg:grid-cols-2">{canViewOrders && <Panel title="Order history">{orders.length ? orders.map((order) => <div className="rounded-lg bg-slate-50 p-3 text-sm" key={order.id}><div className="flex justify-between"><b>{order.number}</b><span>{order.status}</span></div><p className="mt-1 text-slate-600">{money(order.totalMinor, order.currency)} · {order.items.map((item) => `${item.productName}${item.editionName ? ` — ${item.editionName} (${item.planName})` : ""}`).join(", ")}</p>{order.invoice && <Link className="mt-2 inline-block font-bold text-[#0b7197]" href={`/dashboard/invoices/${order.invoice.id}`}>View invoice</Link>}{order.status === "PENDING" && account.lifecycleState === "ACTIVE" && <PendingOrderActions orderId={order.id} canContinue={order.attempts[0]?.status === "PENDING" && Boolean(order.attempts[0]?.checkoutUrl)}/>}</div>) : <Empty/>}</Panel>}{canViewSubscriptions && <Panel title="Subscriptions">{subscriptions.length ? subscriptions.map((subscription) => <div className="rounded-lg bg-slate-50 p-3 text-sm" key={subscription.id}><div className="flex justify-between"><b>{subscription.product.name}{subscription.edition ? ` — ${subscription.edition.name}` : ""}</b><span>{subscription.status}</span></div><p>{subscription.purchasePlan ? purchasePlanLabel(subscription.purchasePlan.type) : "Legacy plan"} · {subscription.seats} seats · through {subscription.currentPeriodEnd.toLocaleDateString()}</p>{subscription.status === "ACTIVE" && account.lifecycleState === "ACTIVE" && <SubscriptionRenewButton subscriptionId={subscription.id} documents={renewalLegal.map(document=>({versionId:document.currentPublishedVersionId!,title:document.title,slug:document.slug}))}/>}</div>) : <Empty/>}</Panel>}</section>
  </div></section>;
}
