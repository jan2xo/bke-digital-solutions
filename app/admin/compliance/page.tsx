...
export default async function CompliancePage() {
  ...
  return <main className="shell py-10"><h1 className="text-4xl font-black">Compliance readiness</h1><p className="mt-2 text-slate-600">Evidence and status tracking only. No entry on this page represents legal, tax, privacy, or regulatory approval.</p><div className="my-6 flex flex-wrap gap-3">{Object.entries(counts).map(([status, count]) => <span key={status} className="rounded-full border px-3 py-2 text-sm font-bold">{status}: {count}</span>)}</div><div className="space-y-4">{requirements.map((r) => <section key={r.id} className="rounded-xl border bg-white p-5 text-slate-900"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase text-slate-500">{r.category}</p><h2 className="text-xl font-black">{r.title}</h2><p className="mt-1 text-slate-600">{r.description}</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">{r.status}</span></div>{r.evidence.length > 0 && <ul className="mt-4 space-y-1 text-sm text-slate-600">{r.evidence.map((e) => <li key={e.id}>Evidence: {e.summary} ({e.kind})</li>)}</ul>}</section>)}</div></main>;
}
