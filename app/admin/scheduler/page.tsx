    ...
    <section className="mt-8 grid gap-5">
      {health.jobs.map((job) => {
        ...
        return <article className="card p-5" key={job.key}>
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-bold uppercase text-primary">{job.category} · {job.health}</p><h2 className="text-xl font-black">{job.name}</h2><p className="text-sm text-muted">{job.description}</p></div><span className="rounded-full bg-slate-700 px-3 py-1 text-sm font-bold text-slate-100">{job.enabled ? "Enabled" : "Paused"}</span></div>
