...
export default async function ObservabilityPage() {
  ...
  const licensing = await db.auditLog.findMany({
    where: { targetType: "LICENSING_ACTIVATION" },
    orderBy: { createdAt: "desc" },
    take: 50,
    select: { id: true, createdAt: true, action: true, targetId: true, metadata: true },
  });
  ...
  return <main className="shell py-10"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase text-primary">Operations</p><h1 className="text-4xl font-black">Platform health</h1><p className="mt-2 text-muted">A single view of application, dependencies, commerce, licensing, security, and recovery readiness.</p></div><span className={`rounded-full px-4 py-2 font-black ${stateClass(snapshot.overall)}`}>{snapshot.overall}</span></div>
    ...
    <section className="card mt-8 p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-black">Licensing traffic</h2><p className="mt-1 text-sm text-muted">Latest Agent activation attempts captured at the Digital Solutions API boundary. No raw license keys are stored.</p></div><span className="admin-status admin-status-info">last 50</span></div>
      {licensing.length === 0 ? <p className="mt-4 text-muted">No licensing activation attempts recorded yet.</p> : <div className="admin-table-wrap mt-4"><table className="admin-table"><thead><tr><th>Time</th><th>Result</th><th>Operation / Request</th><th>Plain-text detail</th></tr></thead><tbody>{licensing.map((entry) => <tr key={entry.id}><td className="whitespace-nowrap">{entry.createdAt.toLocaleString()}</td><td><span className={`admin-status ${entry.action.endsWith("ALLOW") ? "admin-status-success" : "admin-status-danger"}`}>{entry.action.endsWith("ALLOW") ? "ALLOW" : "DENY"}</span></td><td>{entry.targetId ?? "—"}</td><td><pre className="whitespace-pre-wrap break-words text-xs">{JSON.stringify(entry.metadata, null, 2)}</pre></td></tr>)}</tbody></table></div>}
    </section>
