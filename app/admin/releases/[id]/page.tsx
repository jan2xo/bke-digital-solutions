...
export default async function ReleaseDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [version, backups, pendingComplianceCount] = await Promise.all([
    db.productVersion.findUnique({ where: { id: versionId }, include: { product: { include: { editions: { include: { purchasePlans: { where: { active: true }, select: { type: true } } } } } }, artifacts: true, commissioningRuns: { orderBy: { createdAt: "desc" }, take: 20 }, approvals: { orderBy: { createdAt: "desc" }, take: 50 }, supplyChainEvidence: { include: { verificationEvidence: true } } } }),
  ...
  return <main className="shell py-10">
    ...
    <section className="card mt-8 p-6"><h2 className="text-2xl font-black">Release readiness</h2><ReleaseEvidenceControls versionId={version.id} blocked={blocked} approvalStatus={currentApproval?.approvedAt ? "Approved" : currentApproval?.reviewedAt ? "Awaiting human approval" : "Awaiting human review"} backupOptions={backups.map((backup) => ({ id: backup.id, verifiedAt: backup.verifiedAt?.toISOString() ?? null }))}/>
    ...
    <section className="card mt-8 p-6"><h2 className="text-2xl font-black">BKE Commissioning</h2><p className="mt-1 text-sm text-slate-600">Machine-generated technical evidence is separate from human compliance, review, and approval.</p><div className="mt-4 grid gap-3">{version.commissioningRuns.map((run) => <div className="rounded border p-3" key={run.id}><div className="flex flex-wrap justify-between gap-2"><strong>{run.classification} · {run.status}</strong><code className="text-xs">{run.artifactSha256}</code></div><p className="mt-2 text-sm">SBOM: {run.sbomStatus} · Dependencies: {run.dependencyStatus} · Migration: {run.migrationCategory} / {run.migrationStatus}</p>{Array.isArray(run.limitations) && run.limitations.length > 0 && <p className="mt-1 text-xs text-amber-800">Limitations: {JSON.stringify(run.limitations)}</p>}</div>)}{version.commissioningRuns.length === 0 && <p>Commissioning is queued after verified artifact intake.</p>}</div></section>
