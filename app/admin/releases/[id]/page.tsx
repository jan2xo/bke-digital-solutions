...
export default async function ReleaseDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell py-10">
    ...
    <section className="card mt-8 p-6"><h2 className="text-2xl font-black">Release readiness</h2>
      ...
      <div className="mt-6 rounded border border-blue-200 bg-blue-50 p-4"><p className="font-bold text-blue-950">Automated release-evidence pipeline</p><p className="mt-1 text-sm text-blue-900">Machine evidence is ingested and verified from the release workflow. Compliance review and release approval remain human-controlled.</p></div><ReleaseEvidenceControls versionId={version.id} blocked={blocked} backupOptions={backups.map((backup) => ({ id: backup.id, verifiedAt: backup.verifiedAt?.toISOString() ?? null }))}/>
    </section>
