...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    if(input.action==="TRANSFER"){
      ...
      if(prepared.status!=="COMPLETED") await issueCommercialLease({licenseKey:decryptLicenseKey(current.keyCiphertext),installationId:input.installationId,deviceId:input.deviceId,operationId,productVersion:sourceLease?.version ?? "0.0.0",action:"TRANSFER",predecessorLeaseId:sourceLease?.leaseId});
