...
export async function POST(request: Request) {
  try {
    ...
    const legacyHash = existing.version.artifacts.map((a) => `${a.id}:${a.sha256}`).sort().join("|");
    ...
    if (input.action === "VERIFY_SIGNATURE") {
      ...
      const valid = verify(null, Buffer.from(legacyHash), publicKey(signerKeyId), Buffer.from(input.signature, "base64"));
      await db.supplyChainVerificationEvidence.create({ data: { evidenceId: existing.id, kind: "SIGNATURE", artifactHash: legacyHash, signerKeyId, result: valid ? "VERIFIED" : "FAILED", failureReason: valid ? undefined : "SIGNATURE_MISMATCH", metadata: { algorithm: "Ed25519", legacy: true } } });
    ...
    if (input.action === "RECORD_SCAN") {
      const scan = await scanArtifact(legacyHash); await db.supplyChainVerificationEvidence.create({ data: { evidenceId: existing.id, kind: "MALWARE_SCAN", artifactHash: legacyHash, scannerId: scan.scannerId, scannerVersion: scan.scannerVersion, result: scan.result, reference: scan.reference, failureReason: scan.failureReason } });
      await db.supplyChainEvidence.update({ where: { id: existing.id }, data: { malwareStatus: scan.result } }); if (scan.result !== "CLEAN") throw new Error("MALWARE_SCAN_NOT_CLEAN");
    }
