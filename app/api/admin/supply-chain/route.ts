...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === "CERTIFY_COMPLIANCE") {
      ...
      const requiredTypes = [...REGISTRATION_LEGAL_TYPES, ...CHECKOUT_LEGAL_TYPES, ...(product.editions.some((edition) => edition.purchasePlans.some((plan) => plan.type === "MONTHLY" || plan.type === "SEMI_ANNUAL" || plan.type === "ANNUAL")) ? SUBSCRIPTION_LEGAL_TYPES : [])];
