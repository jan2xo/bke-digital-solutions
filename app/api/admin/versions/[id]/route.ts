  ...
  try {
    ...
    if (input.lifecycle) {
      ...
      if (to >= stages.indexOf("STABLE")) {
        ...
        if (!current.approvals[0]?.reviewedById || current.approvals[0].reviewedById === admin.id) throw new Error("RELEASE_SEPARATION_REQUIRED");
      }
    }
    const version = await db.$transaction(async (tx) => {
      ...
      if (input.reviewed || input.approve) await tx.releaseApproval.create({ data: { versionId: id, stage: input.lifecycle ?? current.lifecycle, createdById: admin.id, reviewedById: input.reviewed ? admin.id : undefined, approvedById: input.approve ? admin.id : undefined, approvedAt: input.approve ? new Date() : undefined } });
      return updated;
    });
    await audit({ actorId: admin.id, action: "RELEASE_LIFECYCLE_CHANGED", targetType: "ProductVersion", targetId: id, metadata: { lifecycle: input.lifecycle, approved: Boolean(input.approve) } }); return NextResponse.json(version);
