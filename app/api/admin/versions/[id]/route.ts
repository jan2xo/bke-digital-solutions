...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (input.reviewed && input.approve) throw new Error("REVIEW_AND_APPROVAL_MUST_BE_SEPARATE");
    if (input.approve && (!payloadApproval?.reviewedById || payloadApproval.reviewedById === admin.id) && !(input.breakGlass && env.ALLOW_BREAK_GLASS === "true")) throw new Error("RELEASE_SEPARATION_REQUIRED");
