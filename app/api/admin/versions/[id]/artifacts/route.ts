  ...
  try {
    ...
    const artifact = await db.$transaction(async (tx) => {
      ...
      if (version.supplyChainEvidence) {
        ...
        await tx.supplyChainEvidence.update({ where: { versionId }, data: { manifestJson: { artifacts }, sbomReference: null, provenanceStatus: "RECORDED", dependencyVerified: false, signatureVerified: false, manifestSignature: null, canonicalPayloadHash: null, signatureKeyId: null, signatureAlgorithm: null, signedAt: null, malwareStatus: "PENDING_SCAN" } });
      }
      return created;
    });
    uploaded = null;
