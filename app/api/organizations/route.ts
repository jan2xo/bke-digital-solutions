...
export async function POST(request: Request) { try { assertSameOrigin(request); const user = await requireUser(); const input = createSchema.parse(await request.json()); const account = await createOrganizationAccount({ actorId: user.id, ...input }); return NextResponse.json({ id: account.id }, { status: 201 }); } catch (error) { return apiError(error); } }
