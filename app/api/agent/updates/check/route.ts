...
export async function POST(request:Request) {
  try {
    ...
    parseSemanticVersion(input.current_version);
    ...
    if(compareSemanticVersions(minimum,release.version)>0) return NextResponse.json({error:"POLICY_GENERATION_FAILED"},{status:500});
    if(updatePolicy==="MAJOR_VERSION" && parseSemanticVersion(input.current_version)[0]!==parseSemanticVersion(release.version)[0]) return NextResponse.json({error:"UPDATE_NOT_ENTITLED"},{status:403});
    ...
    const policy={schema:"bke.update-policy.v1",product_id:input.product_id,current_version:input.current_version,latest_version:release.version,minimum_supported_version:minimum,channel:input.channel,platform:release.operatingSystem,architecture:release.architecture,release_id:release.id,artifact_id:artifact.id,artifact_sha256:artifact.sha256.toLowerCase(),artifact_size:Number(artifact.sizeBytes),content_type:artifact.contentType,published_at:release.publishedAt?.toISOString(),issued_at:new Date().toISOString(),revision:release.releasedAt.getTime(),signing_key_id:key.keyId,algorithm:"Ed25519"};
    if(Object.values(policy).some(value=>value===undefined) || policy.artifact_size<0 || !/^[0-9a-f]{64}$/.test(policy.artifact_sha256)) return NextResponse.json({error:"POLICY_GENERATION_FAILED"},{status:500});
