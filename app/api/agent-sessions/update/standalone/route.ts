...
export async function POST(request: Request) {
  try {
    ...
    if (!(await rateLimit(
      `agent-session:standalone-update:${clientIp(request)}`,
      120,
      3600,
    )).allowed) {
    ...
    const authority = await db.$transaction(async (tx) => {
      ...
      if (!product.installable || !product.latestVersion || !valid(product.latestVersion)) {
      ...
      if (!gt(product.latestVersion, input.current_version)) {
        return {
      ...
      return {
        ...
        currentVersion: input.current_version,
        targetVersion: product.latestVersion,
        repository,
        platform: input.platform,
        architecture: input.architecture,
      };
    ...
    switch (authority.status) {
      ...
      case "up_to_date":
        return json({
          status: "up_to_date",
          current_version: authority.currentVersion,
          latest_version: authority.latestVersion,
        });
      case "update_available": {
        const policy = await issueAgentAccountUpdatePolicy({
