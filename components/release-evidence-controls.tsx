...
export function ReleaseEvidenceControls({ versionId, blocked, backupOptions = [] }: { versionId: string; blocked: EvidenceKind[]; backupOptions?: BackupOption[] }) {
  ...
  async function certifyCompliance() { setBusy("COMPLIANCE"); setError(""); try { const response = await fetch("/api/admin/supply-chain", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ versionId, action: "CERTIFY_COMPLIANCE", reviewerIdentity, reviewerRole, scope, attestation: true }) }); if (!response.ok) throw new Error("Compliance certification failed"); router.refresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Compliance certification failed"); } finally { setBusy(null); } }
  const backupBlocked = blocked.includes("BACKUP");
  const complianceBlocked = blocked.includes("COMPLIANCE");
  return <div className="mt-4 rounded border border-dashed p-4"><p className="text-sm font-bold">Record evidence</p><p className="mt-1 text-xs text-slate-600">Upload generated documents or certify an eligible verified backup. The server computes the current payload hash and validates all evidence.</p>{backupBlocked && <div className="mt-3 flex flex-wrap items-center gap-3"><select className="input" aria-label="Verified backup" value={selectedBackup} onChange={(event) => setSelectedBackup(event.target.value)}><option value="">Select verified backup</option>{backupOptions.map((backup) => <option key={backup.id} value={backup.id}>{backup.id} · verified {backup.verifiedAt ?? "unknown"}</option>)}</select><button className="rounded-lg border border-[#2D5579] px-3 py-1.5 text-xs font-bold text-[#213A53]" type="button" disabled={busy !== null || !selectedBackup} onClick={() => { void certifyBackup(selectedBackup); }}>{busy === "BACKUP" ? "Certifying…" : "Certify Backup for Release"}</button></div>}{complianceBlocked && <div className="mt-3 grid gap-2 rounded border p-3"><p className="text-xs text-slate-600">This is a human attestation. The server re-reads the current published legal documents and binds the certification to this release payload.</p><input className="input" aria-label="Compliance reviewer" placeholder="Reviewer identity" value={reviewerIdentity} onChange={(event) => setReviewerIdentity(event.target.value)} /><input className="input" aria-label="Compliance role" placeholder="Reviewer role" value={reviewerRole} onChange={(event) => setReviewerRole(event.target.value)} /><input className="input" aria-label="Compliance scope" placeholder="Review scope" value={scope} onChange={(event) => setScope(event.target.value)} /><label className="text-xs"><input type="checkbox" checked={attested} onChange={(event) => setAttested(event.target.checked)} /> I confirm the required legal, privacy, tax, and retention review assertions for this release.</label><button className="w-fit rounded-lg border border-[#2D5579] px-3 py-1.5 text-xs font-bold text-[#213A53]" type="button" disabled={busy !== null || !reviewerIdentity || !reviewerRole || !scope || !attested} onClick={() => { void certifyCompliance(); }}>{busy === "COMPLIANCE" ? "Certifying…" : "Certify Compliance"}</button></div>}{<div className="mt-3 flex flex-wrap gap-3">{blocked.filter((kind) => kind !== "BACKUP" && kind !== "COMPLIANCE").map((kind) => <label key={kind} className="rounded-lg border border-[#2D5579] px-3 py-1.5 text-xs font-bold text-[#213A53]">{busy === kind ? "Recording…" : `Upload ${kind}`}<input className="sr-only" type="file" onChange={(event) => { void record(kind, event.target.files?.[0]); event.currentTarget.value = ""; }} disabled={busy !== null}/></label>)}</div>}{error && <p className="mt-2 text-sm text-red-700">{error}</p>}</div>;
}
