...
export function createWebPaymentsAdapter(): WebPaymentsAdapter {
  ...
  if (source !== "environment") {
    throw new Error("V2_DATABASE_PROVIDER_CONFIGURATION_NOT_MIGRATED");
  }
  ...
  return createPayMongoPaymentsAdapter({
    ...
    successUrl: (input) =>
      `${origin}/checkout/success?order=${encodeURIComponent(input.commercialReference)}`,
    cancelUrl: (input) =>
      `${origin}/checkout/cancel?order=${encodeURIComponent(input.commercialReference)}`,
  });
}
