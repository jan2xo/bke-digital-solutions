...
const allowedModels = new Set(["LegalDocumentVersion", "LegalAcceptance"]);
...
const requiredPaths = [
  ...
  "prisma/migrations/0001_legal_acceptance_baseline/migration.sql",
  "contracts",
  "logic",
  "prisma/repositories",
  "test",
  ...
  for (const specifier of importsFrom(source)) {
    if (specifier.startsWith(".")) {
      ...
      if (!isWithin(moduleRoot, target)) {
        violations.push(`${sourcePath} escapes Legal through ${specifier}`);
        continue;
      }
...
if (existsSync(schemaPath)) {
  ...
  for (const model of models) {
    if (!allowedModels.has(model)) violations.push(`Legal Prisma owns foreign model ${model}`);
  }
  for (const expected of allowedModels) {
    if (!models.includes(expected)) violations.push(`Legal Prisma is missing owned model ${expected}`);
  }
}
...
console.log(
  `Legal extraction boundary GREEN: ${packageSources.length} package-owned source files; host module.ts and module-composition.test.ts excluded`,
);
