...
if (!connectionString) {
  throw new Error("DATABASE_URL is required for Identity recent-auth completion certification.");
}
...
async function createChallenge(input: {
  ...
  await client.query(
    ...
    [
      input.id,
      input.userId,
      input.purpose ?? "RECENT_AUTH",
      proofProvider.hashChallengeToken(input.token),
      hashEmailCode(input.code),
...
try {
  await seedUser({
    id: "recent-customer",
    role: "CUSTOMER",
    sessionId: "recent-customer-session",
    tokenHash: "customer-token-hash",
  ...
  const customer = await customerCapability.complete({
    sessionToken: "customer-boundary-token",
    password,
  });
  if (
    customer.status !== "COMPLETED" ||
    customer.verificationMethod !== "PASSWORD" ||
    customer.session.assuranceLevel !== "RECENTLY_AUTHENTICATED" ||
    customer.session.recentAuthenticatedAt?.getTime() !== now.getTime()
  ) {
  ...
  await seedUser({
    id: "recent-admin",
    role: "ADMIN",
    sessionId: "recent-admin-session",
    tokenHash: "admin-token-hash",
    assuranceLevel: "MFA_VERIFIED",
  });
  await createChallenge({
    id: "recent-admin-login-challenge",
    userId: "recent-admin",
    token: "unrelated-login-token",
    code: "111111",
    purpose: "LOGIN",
  });
  await createChallenge({
    id: "recent-admin-challenge",
    userId: "recent-admin",
    token: "recent-admin-token",
    code: "123456",
  });
  ...
  const admin = await adminCapability.complete({
    sessionToken: "admin-boundary-token",
    password,
    challengeToken: "recent-admin-token",
    code: "123456",
  });
  if (
    admin.status !== "COMPLETED" ||
    admin.verificationMethod !== "PASSWORD_EMAIL_OTP" ||
    admin.session.assuranceLevel !== "RECENTLY_AUTHENTICATED"
  ) {
    throw new Error(`Admin email-OTP recent-auth failed: ${JSON.stringify(admin)}`);
  }
  const purposeIsolation = await client.query<{
    recentConsumedAt: Date | null;
    loginConsumedAt: Date | null;
  }>(
    `SELECT
  ...
  if (!purposeIsolation.rows[0]?.recentConsumedAt || purposeIsolation.rows[0]?.loginConsumedAt) {
    throw new Error("RECENT_AUTH completion violated MFA challenge purpose isolation.");
  }

  await seedUser({
    id: "recent-rollback-admin",
    role: "ADMIN",
    sessionId: "recent-rollback-session",
  ...
  await client.query(
    `INSERT INTO "AdministratorRecoveryCode" ("id", "userId", "codeHash")
     VALUES ($1, $2, $3)`,
    ["recent-rollback-recovery", "recent-rollback-admin", proofProvider.hashRecoveryCode(recoveryCode)],
  );
  await createChallenge({
    id: "recent-rollback-challenge",
    userId: "recent-rollback-admin",
    token: "recent-rollback-token",
    code: "654321",
  });
  ...
  const rolledBack = await rollbackCapability.complete({
    sessionToken: "rollback-boundary-token",
    password,
    challengeToken: "recent-rollback-token",
    code: recoveryCode,
  });
  if (rolledBack.status !== "FAILED" || rolledBack.code !== "PERSISTENCE_UNAVAILABLE") {
    throw new Error(`Forced transaction failure did not fail closed: ${JSON.stringify(rolledBack)}`);
  }
  ...
  if (
    !rollback ||
    rollback.challengeConsumedAt !== null ||
    rollback.recoveryUsedAt !== null ||
    rollback.recentAuthenticatedAt !== null ||
    rollback.assuranceLevel !== "MFA_VERIFIED"
  ) {
  ...
  await client.query(
    `DROP TRIGGER IF EXISTS identity_recent_auth_cert_fail_session_update_trigger ON "Session"`,
  ).catch(() => undefined);
  await client.query(
    `DROP FUNCTION IF EXISTS identity_recent_auth_cert_fail_session_update()`,
  ).catch(() => undefined);
  await client.end();
}
