...
try {
  const tables = await client.query<{ table_name: string }>(`
    SELECT table_name
     ...
     WHERE table_schema = 'public'
       ...
       AND table_name <> '_bke_module_migrations'
     ORDER BY table_name
  `);
  ...
  const ledger = await client.query<{ moduleId: string; migrationName: string; state: string }>(`
    SELECT "moduleId", "migrationName", "state"
      FROM "_bke_module_migrations"
     ORDER BY "moduleId", "migrationName"
  `);

  if (
    ledger.rows.length !== 1 ||
    ledger.rows[0]?.moduleId !== "identity" ||
    ledger.rows[0]?.migrationName !== "0001_identity_baseline" ||
    ledger.rows[0]?.state !== "APPLIED"
  ) {
    throw new Error(`Unexpected Identity migration ledger: ${JSON.stringify(ledger.rows)}`);
  }
