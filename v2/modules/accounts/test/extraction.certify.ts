...
function isPackageSource(sourceFile: string) {
  ...
  if (path === "module.ts" || path === "test/module-composition.test.ts") return false;
  return (
...
const requiredPaths = [
  ...
  "prisma/migrations/0001_accounts_baseline/migration.sql",
  "contracts",
  "logic",
  "providers",
  "prisma/repositories",
...
for (const path of requiredPaths) {
  if (!existsSync(resolve(moduleRoot, path))) {
    violations.push(`missing extraction-owned path ${path}`);
  }
}
  ...
  for (const specifier of importsFrom(source)) {
    if (specifier.startsWith(".")) {
      ...
      if (!isWithin(moduleRoot, target)) {
        violations.push(`${sourcePath} escapes Accounts through ${specifier}`);
        continue;
      }
...
if (existsSync(schemaPath)) {
  ...
  for (const model of models) {
    if (!allowedModels.has(model)) violations.push(`Accounts Prisma owns foreign model ${model}`);
  }
  for (const expected of allowedModels) {
    if (!models.includes(expected)) violations.push(`Accounts Prisma is missing owned model ${expected}`);
  }
}
...
for (const persistenceFile of persistenceFiles) {
  ...
  for (const match of source.matchAll(tableReferencePattern)) {
    ...
    if (!allowedModels.has(table)) {
      violations.push(
        `${normalize(relative(moduleRoot, persistenceFile))} references foreign table ${table}`,
      );
    }
  }
}
...
if (violations.length > 0) {
  console.error("Accounts extraction boundary violations:");
  for (const violation of violations) console.error(`- ${violation}`);
  process.exit(1);
}

console.log(
  `Accounts extraction boundary GREEN: ${packageSources.length} package-owned source files; host module.ts and module-composition.test.ts excluded`,
);
