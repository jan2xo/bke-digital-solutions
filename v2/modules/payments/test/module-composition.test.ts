...
const refundProvider: PaymentsRefundProvider = {
  name: "composition-provider",
  async createRefund() {
    throw new Error("composition startup must not invoke refund provider");
  },
};
...
describe("Payments module composition", () => {
  it("registers all certified Payments capabilities without touching dependencies at startup", async () => {
    ...
    expect(application.has(PAYMENTS_REFUND_INITIATION_CAPABILITY_ID)).toBe(true);
    expect(
      typeof application.get<PaymentsCheckoutAttemptCapability>(PAYMENTS_CHECKOUT_ATTEMPT_CAPABILITY_ID).create,
    ).toBe("function");
    expect(
      typeof application.get<PaymentsProviderEventIngestionCapability>(PAYMENTS_PROVIDER_EVENT_INGESTION_CAPABILITY_ID).ingest,
    ).toBe("function");
    expect(
      typeof application.get<PaymentsSettlementFactCapability>(PAYMENTS_SETTLEMENT_FACT_CAPABILITY_ID).reconcile,
    ).toBe("function");
    expect(
      typeof application.get<PaymentsRefundInitiationCapability>(PAYMENTS_REFUND_INITIATION_CAPABILITY_ID).initiate,
    ).toBe("function");
  });
});
