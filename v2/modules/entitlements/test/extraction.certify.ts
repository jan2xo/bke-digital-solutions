...
const allowedModels = new Set(["Entitlement"]);
...
const requiredPaths = [
  ...
  "prisma/migrations/0001_entitlements_durable_right_baseline/migration.sql",
  "contracts",
  "logic",
  "prisma/repositories",
  "test",
  ...
  for (const specifier of importsFrom(source)) {
    if (specifier.startsWith(".")) {
      ...
      if (!isWithin(moduleRoot, target)) {
        violations.push(`${sourcePath} escapes Entitlements through ${specifier}`);
        continue;
      }
...
if (existsSync(schemaPath)) {
  ...
  for (const model of models) {
    if (!allowedModels.has(model)) violations.push(`Entitlements Prisma owns foreign model ${model}`);
  }
  for (const expected of allowedModels) {
    if (!models.includes(expected)) violations.push(`Entitlements Prisma is missing owned model ${expected}`);
  }
}
...
if (violations.length > 0) {
  console.error("Entitlements extraction boundary violations:");
  for (const violation of violations) console.error(`- ${violation}`);
  process.exit(1);
}

console.log(
  `Entitlements extraction boundary GREEN: ${packageSources.length} package-owned source files; host module.ts and module-composition.test.ts excluded`,
);
