...
describe("Digital Solutions identity and Cloud-Agent contract", () => {
  ...
  it("validates the exact signed lease envelope and strict payload", async () => {
    const { leaseEnvelopeSchema, leasePayloadSchema, parseLeaseEnvelope } = await import("@/apps/web/licensing/cloud-agent-contract");
  ...
  it("enforces the versioned cloud boundary and lifecycle request rules", async () => {
    const { CLOUD_AGENT_PROTOCOL_VERSION, requireCloudAgentVersion, cloudAgentRequestSchema, validateLifecycleRequest, CloudAgentProtocolError } = await import("@/apps/web/licensing/cloud-agent-contract");
