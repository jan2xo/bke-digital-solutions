...
vi.mock("@/platform/host/env", () => ({ env: { SUPPLY_CHAIN_SIGNING_KEY_ID: "supply-test" } }));
...
describe("historical release evidence contract", () => {
...
describe("release-evidence ingestion retirement", () => {
  it("makes GitHub the release authority instead of persisting certification evidence", async () => {
    const source = await (await import("node:fs/promises")).readFile("app/api/release-evidence/ingest/route.ts", "utf8");
    expect(source).toContain("RELEASE_EVIDENCE_INGESTION_RETIRED");
    expect(source).toContain("GitHub is the software release authority");
    expect(source).toContain("status: 410");
    expect(source).not.toContain("downloadObject");
    expect(source).not.toContain("supplyChainEvidence");
  });
});
...
describe("historical automatic verified-backup selection", () => {
