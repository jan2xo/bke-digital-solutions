...
test("license reveal and transfer confirmations use protected operations", async ({ page }) => {
  ...
  const product = await db.product.create({ data: { slug: `phase5-license-${suffix}`, name: `License ${suffix}`, summary: "Certification fixture", description: "Certification fixture", type: "SOFTWARE", active: true } });
