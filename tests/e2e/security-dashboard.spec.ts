...
test("administrator manages only their own sessions and revocation takes effect on the next request", async ({ page, browser }) => {
  ...
  const { secret } = await enrollAndLoginAdmin(page, email, password);
  const secondContext = await browser.newContext(); const secondPage = await secondContext.newPage();
  await secondPage.goto("/login"); await secondPage.getByLabel("Email address").first().fill(email); await secondPage.locator('input[name="password"]').fill(password); await secondPage.getByRole("button", { name: "Sign in" }).click(); await secondPage.getByLabel("Authenticator or recovery code").fill(totpCode(secret)); await secondPage.getByRole("button", { name: "Verify and sign in" }).click();
