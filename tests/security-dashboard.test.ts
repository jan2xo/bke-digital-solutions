...
vi.mock("@/platform/host/env", () => ({ env: { SESSION_SECRET: "security-dashboard-test-secret-value", TRUST_PROXY_HOPS: 1 } }));

describe("security dashboard safety", () => {
  it("summarizes user agents without returning the raw value", async () => {
    const { summarizeUserAgent } = await import("@/platform/host/security/session-display");
