...
// Final owner-adoption parity runs against released Commerce 0.14.0 and Payments owner contracts.
...
describe("payment webhook owner-adoption parity", () => {
  ...
  it("locks one transaction-bound Payments → Commerce → Entitlements settlement graph", () => {
    ...
    expect(settlementSource).toContain("createTransactionalClaimUnitIssuer(tx)");
    expect(settlementSource).toContain('"fulfillmentMode"::text AS "fulfillmentMode"');
    expect(settlementSource).toContain('"fulfillmentSnapshot"');
