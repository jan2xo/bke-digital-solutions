...
export async function processCommissioningRun(runId: string) {
  ...
  const result: CommissioningEvidence = { schema: "bke.commissioning.evidence.v1", result: classification === "SCRIPT" || classification === "ZIP_ARCHIVE" ? "PARTIAL" : "UNDETERMINED", artifactSha256: run.artifact.sha256, artifactId: run.artifact.id, generatedAt: nowIso(), generator: COMMISSIONING_GENERATOR_VERSION, method: "BKE metadata-only fallback", components: [component], dependencies: [], classification, limitations };
