...
import { COMMISSIONING_GENERATOR_VERSION, COMMISSIONING_POLICY_VERSION, classifyArtifact, type CommissioningEvidence } from "@/lib/commissioning/types";
...
export async function processCommissioningRun(runId: string) {
  ...
  const result: CommissioningEvidence = { schema: "bke.commissioning.evidence.v1", result: run.classification === "SCRIPT" || run.classification === "ZIP_ARCHIVE" ? "PARTIAL" : "UNDETERMINED", artifactSha256: run.artifact.sha256, artifactId: run.artifact.id, generatedAt: nowIso(), generator: COMMISSIONING_GENERATOR_VERSION, method: "BKE static canonical-byte inspection", components: [component], dependencies: [], classification: run.classification, limitations };
  ...
  await db.$transaction(async (tx) => {
    ...
    await tx.supplyChainVerificationEvidence.createMany({ data: evidenceItems.map((item) => ({ evidenceId: existing.id, kind: item.kind, result: item.result, artifactHash: item.artifactHash, scannerId: COMMISSIONING_GENERATOR_VERSION, scannerVersion: COMMISSIONING_GENERATOR_VERSION, metadata: item.metadata })) });
    ...
    await tx.commissioningRun.update({ where: { id: run.id }, data: { status: "EVIDENCE_READY", completedAt: new Date(), sbomStatus: result.result, dependencyStatus: "NONE_OBSERVED", migrationCategory: migration.category, migrationStatus: migration.status, evidence: { sbom: result, dependencies: dependency, provenance, migration }, limitations } });
  });
