...
export async function completeLoginChallenge(code: string, request: Request) {
  ...
  jar.delete(COOKIE); await createSession(challenge.userId, request, { mfaVerified: true, recent: true }); return { userId: challenge.userId, recoveryUsed: Boolean(recovery) };
}
