      ...
      return await db.$transaction(async (tx) => {
        ...
        if (!operation) {
          ...
          if (operation.status === "COMPLETED" && operation.resultLeaseId) {
            ...
            if (input.action === "TRANSFER" && (metadata.installationId !== input.installationId || metadata.deviceId !== input.deviceId || (input.predecessorLeaseId && metadata.predecessorLeaseId !== input.predecessorLeaseId))) throw new Error("OPERATION_INPUT_MISMATCH");
