...
export function issueSignedLease(payload: LeasePayload) {
  if (env.NODE_ENV === "production" && (!env.LICENSE_SIGNING_PRIVATE_KEY || !env.LICENSE_SIGNING_PUBLIC_KEY)) throw new Error("LEASE_SIGNING_NOT_CONFIGURED");
  const serialized = canonical(payload).toString("utf8");
  return { payload: serialized, signature: sign(null, Buffer.from(serialized), key(env.LICENSE_SIGNING_PRIVATE_KEY, "private")).toString("base64"), key_id: payload.key_id, algorithm: "Ed25519" as const };
}
export function verifySignedLease(lease: ReturnType<typeof issueSignedLease>) { return Boolean(lease.algorithm === "Ed25519" && verify(null, Buffer.from(lease.payload), key(env.LICENSE_SIGNING_PUBLIC_KEY, "public"), Buffer.from(lease.signature, "base64"))); }
