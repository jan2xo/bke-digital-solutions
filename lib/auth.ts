...
export async function createSession(userId: string, request?: Request, options?: { mfaVerified?: boolean; recent?: boolean; authenticationMethod?: "PASSWORD" | "PASSWORD_TOTP" | "PASSWORD_EMAIL_OTP" | "PASSWORD_RECOVERY" | "MAGIC_LINK" | "MFA_ENROLLMENT" }) {
  ...
  const session = await db.$transaction(async (tx) => { const user=await tx.user.findUniqueOrThrow({where:{id:userId},select:{role:true,email:true,suspendedAt:true,lifecycleState:true}});if(user.role!=="ADMIN"&&(user.suspendedAt||user.lifecycleState!=="ACTIVE"))throw new Error("ACCOUNT_NOT_ACTIVE");const created=await tx.session.create({ data: { userId, tokenHash: hashToken(token), expiresAt, absoluteExpiresAt: expiresAt, lastSeenAt: now, mfaVerifiedAt: options?.mfaVerified ? now : null, recentAuthenticatedAt: options?.recent ? now : null, userAgentSummary: summarizeUserAgent(request?.headers.get("user-agent")), networkHint: safeNetworkHint(request ? clientIp(request) : "unknown"), authenticationMethod: options?.authenticationMethod ?? "PASSWORD", assuranceLevel: options?.recent ? "RECENTLY_AUTHENTICATED" : options?.mfaVerified ? "MFA_VERIFIED" : "BASIC" } });if(user.role==="ADMIN")await queueSecurityEmail(tx,{type:"SECURITY_NEW_SESSION",recipient:user.email,subject:"New BKE administrator session",deduplicationKey:`security-new-session:${created.id}`});return created; });
