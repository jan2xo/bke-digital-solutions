...
export const emailProvider:EmailProvider=env.RESEND_API_KEY?new ResendProvider():new DevelopmentProvider();
...
export async function queueCommerceEmail(tx:Prisma.TransactionClient,input:{type:string;recipient:string;subject:string;payload:Record<string,unknown>}){await tx.emailOutbox.create({data:{...input,payload:input.payload as Prisma.InputJsonValue}})}
function render(type:string,payload:Record<string,unknown>){const order=String(payload.orderNumber??"");const invoice=String(payload.invoiceNumber??"");return type==="PAYMENT_RECEIPT"?`Payment for order ${order} was confirmed.`:type==="INVOICE_ISSUED"?`Commercial invoice ${invoice} is available in your customer portal.`:type==="LICENSE_ISSUED"?`Your license for order ${order} is ready. Reveal the full key once in your secure portal.`:type==="PAYMENT_FAILED"?`Payment for order ${order} failed. No license was issued.`:type==="REFUND_CONFIRMED"?`The refund for order ${order} was confirmed and its access was revoked.`:"BKE Digital Solutions account notification."}
