...
function render(type:string,payload:Record<string,unknown>){const order=String(payload.orderNumber??"");const invoice=String(payload.invoiceNumber??"");return type==="PAYMENT_RECEIPT"?`Payment for order ${order} was confirmed.`:type==="INVOICE_ISSUED"?`Commercial invoice ${invoice} is available in your customer portal.`:type==="LICENSE_ISSUED"?`Your license for order ${order} is ready. View the full key in your secure portal.`:type==="PAYMENT_FAILED"?`Payment for order ${order} failed. No license was issued.`:type==="REFUND_CONFIRMED"?`The refund for order ${order} was confirmed and its access was revoked.`:"BKE Digital Solutions account notification."}
