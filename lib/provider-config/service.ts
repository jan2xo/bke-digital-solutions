...
export async function safeProviderStatuses() {
  const configs = await db.externalProviderConfiguration.findMany({ select: { id: true, provider: true, environment: true, enabled: true, senderName: true, senderEmail: true, supportEmail: true, validationStatus: true, lastValidationCode: true, lastValidatedAt: true, lastRotatedAt: true, credentials: { where: { revokedAt: null }, select: { credentialType: true, maskedHint: true, createdAt: true } } }, orderBy: [{ provider: "asc" }, { environment: "asc" }] });
  const [success, failure] = await Promise.all([db.emailOutbox.findFirst({ where: { status: "SENT" }, orderBy: { sentAt: "desc" }, select: { sentAt: true } }), db.emailOutbox.findFirst({ where: { status: { in: ["FAILED", "PERMANENTLY_FAILED"] }, lastError: { not: null } }, orderBy: { createdAt: "desc" }, select: { createdAt: true, lastError: true } })]);
  return configs.map((config) => config.provider === "RESEND" ? { ...config, deliveryHealth: failure && (!success?.sentAt || failure.createdAt > success.sentAt) ? "DEGRADED" : success ? "HEALTHY" : "UNKNOWN", lastSuccessfulDeliveryAt: success?.sentAt ?? null, lastDeliveryFailureCategory: failure?.lastError ?? null, lastDeliveryFailureAt: failure?.createdAt ?? null } : { ...config, deliveryHealth: null, lastSuccessfulDeliveryAt: null, lastDeliveryFailureCategory: null, lastDeliveryFailureAt: null });
}
