...
The reusable metrics endpoint is `GET /api/health/metrics`. It returns typed health cards for the application, PostgreSQL, Valkey, object storage, scheduler, backups, payments, licensing issuance, email, security, and infrastructure. Health is `HEALTHY`, `WARNING`, or `CRITICAL`.
