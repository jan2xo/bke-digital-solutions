...
docker compose --env-file .env -f docker-compose.production.yml --profile self-hosted-storage up -d minio minio-init
...
Populate `.env` from the owner secret store; never use placeholders. MinIO
