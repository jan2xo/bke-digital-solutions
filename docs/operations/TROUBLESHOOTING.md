...
| deployment preflight fails | Compose/topology/restart policy mismatch | run `npm run ops:validate`, correct config, and rerun | stop deployment |
...
| manifest validator rejects Caddy domain | verifier expected one obsolete token spelling | run current `npm run ops:validate`; validator accepts supported environment-token wiring | fixed |
| dependency evidence generation fails | isolated `npm ci`, resolution, or audit is not certifiable | preserve the JSON result, correct dependency state/network, and rerun | do not record VERIFIED |
