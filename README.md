...
The mock payment provider is the safe local default. Development verification and magic-link URLs are printed to the server console. Set `RESEND_API_KEY` only when using a verified sender.
...
See [architecture](docs/architecture.md), [authorization matrix](docs/authorization.md), [payment flow](docs/payment-flow.md), [threat model](docs/threat-model.md), and the [production deployment checklist](docs/deployment-checklist.md).
