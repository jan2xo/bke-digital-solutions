...
type BroadcastRow = Readonly<{ id: string; value: string; updatedAt: Date }>;

type ProductIdentityRow = Readonly<{ id: string }>;
...
async function requireActiveProduct(productId: string): Promise<ProductIdentityRow> {
  const result = await getPostgresPool().query<ProductIdentityRow>(
    `SELECT "id"
       FROM "Product"
      WHERE "productId" = $1
        AND "active" = TRUE
...
export async function publishProductBroadcast(
  ...
  try {
    ...
    await transaction.query(
      ...
      [
        randomUUID(),
        storageKey(record.productId, record.code),
        BROADCAST_GROUP,
...
export async function withdrawProductBroadcast(
  ...
  try {
    await transaction.query("BEGIN");
    const result = await transaction.query<BroadcastRow>(
    ...
    await transaction.query(
      `UPDATE "SiteContent"
          SET "value" = $1,
              ...
              "updatedAt" = NOW()
        WHERE "key" = $3`,
      [JSON.stringify(withdrawn), actorId, key],
    );
    await auditInTransaction(transaction, {
      actorId,
...
export async function listProductBroadcasts(productId?: string): Promise<ProductBroadcastRecord[]> {
  ...
  const result = await getPostgresPool().query<BroadcastRow>(
    `SELECT "id", "value", "updatedAt"
       FROM "SiteContent"
      WHERE "group" = $1
