...
async function main() {
  const product = await db.product.upsert({
    ...
    create: { slug: PRODUCT_SLUG, name: "BKE Agent Integration Test Product", summary: "Local Agent integration fixture", description: "TEST ONLY", type: "SOFTWARE", active: true },
  });
  ...
  const plan = await db.purchasePlan.upsert({ where: { editionId_type: { editionId: edition.id, type: "PERPETUAL" } }, update: { active: true, amountMinor: 0 }, create: { editionId: edition.id, type: "PERPETUAL", amountMinor: 0, currency: "PHP", active: true } });
  ...
  const licenseKey = "BKE-TEST-" + randomBytes(20).toString("hex").toUpperCase().match(/.{1,5}/g)!.join("-");
  ...
  console.info(JSON.stringify({ testOnly: true, productId: product.id, productSlug: product.slug, versionId: version.id, version: version.version, licenseId: license.id, licenseKey: existing?.keyCiphertext ? decryptLicenseKey(existing.keyCiphertext) : licenseKey }));
}
