...
async function main() {
  ...
  const plan = await db.purchasePlan.upsert({ where: { editionId_type: { editionId: edition.id, type: "PERPETUAL" } }, update: { active: true }, create: { editionId: edition.id, type: "PERPETUAL", amountMinor: 0, currency: "PHP", renewalBehavior: "NONE", active: true } });
  const price = await db.price.upsert({ where: { id: "certification-price" }, update: { active: true }, create: { id: "certification-price", productId: product.id, licensePolicyId: policy.id, name: "Certification", amountMinor: 0, currency: "PHP", billingType: "ONE_TIME", active: true } });
