...
async function main() {
  const product = await db.product.upsert({
    where: { slug: "bke-certification-product" },
    update: { active: true, publishedAt: new Date() },
    create: { slug: "bke-certification-product", name: "BKE Certification Product", summary: "CI fixture", description: "Disposable certification fixture", type: "SOFTWARE", active: true, publishedAt: new Date() },
  });
  const policy = await db.licensePolicy.upsert({
    where: { id: "certification-policy" },
    update: {},
    create: { id: "certification-policy", productId: product.id, name: "Certification", maxSeats: 1, maxDevicesPerSeat: 2, validityDays: null },
  });
  const edition = await db.edition.upsert({
    where: { productId_slug: { productId: product.id, slug: "certification" } },
    update: { active: true },
    create: { productId: product.id, slug: "certification", name: "Certification", updatePolicy: "LIFETIME", active: true },
  });
  const plan = await db.purchasePlan.upsert({
    where: { editionId_type: { editionId: edition.id, type: "PERPETUAL" } },
    update: { active: true },
    create: { editionId: edition.id, type: "PERPETUAL", amountMinor: 0, currency: "PHP", renewalBehavior: "NONE", active: true },
  });
  const price = await db.price.upsert({
    where: { id: "certification-price" },
    update: { active: true },
    create: { id: "certification-price", productId: product.id, licensePolicyId: policy.id, name: "Certification", amountMinor: 0, currency: "PHP", billingType: "ONE_TIME", active: true },
  });
  const v1 = await db.productVersion.upsert({
    where: { productId_version: { productId: product.id, version: "1.0.0" } },
    update: { active: true, isLatest: false },
    create: { productId: product.id, version: "1.0.0", channel: "STABLE", lifecycle: "STABLE", operatingSystem: "linux", architecture: "x86_64", active: true, isLatest: false, publishedAt: new Date(), releasedAt: new Date(Date.now() - 10000) },
  });
  const v2 = await db.productVersion.upsert({
    where: { productId_version: { productId: product.id, version: "2.0.0" } },
    update: { active: true, isLatest: true, lifecycle: "STABLE", minimumSupportedVersion: "1.0.0" },
    create: { productId: product.id, version: "2.0.0", channel: "STABLE", lifecycle: "STABLE", operatingSystem: "linux", architecture: "x86_64", active: true, isLatest: true, minimumSupportedVersion: "1.0.0", publishedAt: new Date(), releasedAt: new Date() },
  });
  const artifact = await db.productArtifact.upsert({
    where: { objectKey: "certification/authority-product-v2.sh" },
    update: { active: true, versionId: v2.id, sha256, sizeBytes: body.length },
    create: { productId: product.id, versionId: v2.id, name: "authority-product-v2.sh", objectKey: "certification/authority-product-v2.sh", sha256, sizeBytes: body.length, contentType: "application/octet-stream", active: true },
  });
